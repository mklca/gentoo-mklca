...
BDEPEND="
	virtual/pkgconfig"
PDEPEND="
	~dev-python/ovs-2.16.0[${PYTHON_USEDEP}]"
...
pkg_setup() {
	if use modules ; then
		...
		kernel_is ge 3 10 0 || die "Linux >= 3.10.0 and <= 5.10 required for userspace modules"
		kernel_is le 5 10 999 || die "Linux >= 3.10.0 and <= 5.10 required for userspace modules"
		linux-mod_pkg_setup
	else
		CONFIG_CHECK+=" ~OPENVSWITCH"
...
src_install() {
	...
	systemd_newunit "${FILESDIR}/ovsdb-server-r4.service" ovsdb-server.service
	systemd_newunit "${FILESDIR}/ovs-vswitchd-r4.service" ovs-vswitchd.service
