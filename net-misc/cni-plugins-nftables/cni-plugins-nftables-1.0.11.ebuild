...
DESCRIPTION="nftables plugins for the container networking infrastructure"
...
SRC_URI="https://github.com/greenpau/cni-plugins/archive/v${PV}.tar.gz -> ${P}.tar.gz
	${EGO_SUM_SRC_URI}"
...
src_compile() {
	mkdir "${WORKDIR}/bin"
	local cmdname
	for cmdname in nftables-firewall nftables-portmap; do
		einfo "Building ${cmdname}"
		CGO_LDFLAGS="$(usex hardened '-fno-PIC ' '')" "${GO:-go}" build -o "${WORKDIR}/bin/${cmdname}" "./cmd/cni-${cmdname}" || die
	done
}
